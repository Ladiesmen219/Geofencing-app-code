...
class _GeoFenceState extends State<GeoFence> {
  ...
  bool attendaceAlreadyMarked = false;
  ...
  // To stop the attendance
    void startGeofenceService() async {
      ...
      _positionStream = Geolocator.getPositionStream().listen((Position position) async { 
        double distanceInMeters = Geolocator.distanceBetween(widget.latitudeCenter, widget.longitudeCenter, position.latitude, position.longitude);

        setState(() {
          isInGeofence = distanceInMeters <= widget.radiusCenter;
        });

        if(isInGeofence && prefs.getInt('count') == 0) {
          if(attendaceAlreadyMarked) {
            showTopSnackBar(Overlay.of(context), CustomSnackBar.error(message: "Attendance already marked"));
            return;
          }
          prefs.setInt('count', 1);
          await firebaseServices.markAttendanceEntry(uId: FirebaseAuth.instance.currentUser!.uid, officeName: widget.name);
          showTopSnackBar(Overlay.of(context), const CustomSnackBar.success(message: "Attendance Started"));
          setState(() {
            colour = Colors.orange.shade700;
            attendaceAlreadyMarked = true;
          });
        } else if(!isInGeofence && prefs.getInt('count') == 1) {
          prefs.setInt('count', 0);
          await firebaseServices.markAttendanceExit(uId: FirebaseAuth.instance.currentUser!.uid);
          showTopSnackBar(Overlay.of(context), const CustomSnackBar.error(message: "Your attendance is stopped!"));

          setState(() {
            colour = Colors.orange.shade100;
          });
        } else if(!isInGeofence) {
          showTopSnackBar(Overlay.of(context), CustomSnackBar.info(message: "You are currently outside the geofence"));
        }
       });
  ...
  //     print("Enter");
  //     print("Distance = $distanceInMeters"); print("Radius = $radiusInMeter");
  ...
  //       });
  //     } else {
  //       showTopSnackBar(Overlay.of(context), const CustomSnackBar.error(message: "Your attendance is already stopped !"));
  //     }
  //   }
  // }
  // To stop the attendance
